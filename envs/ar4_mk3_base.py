...
class MujocoPyFetchEnv(get_base_fetch_env(MujocoPyRobotEnv)):
    def _step_callback(self):
        if self.block_gripper:
            self.sim.data.set_joint_qpos("gripper_jaw1_joint", 0.0)
            self.sim.data.set_joint_qpos("gripper_jaw2_joint", 0.0)
            self.sim.forward()
    ...
    def _env_setup(self, initial_qpos):
        ...
        ) + self.sim.data.get_site_xpos("grip")
        gripper_rotation = np.array([1.0, 0.0, 1.0, 0.0])
        self.sim.data.set_mocap_pos("robot_mocap", gripper_target)
        self.sim.data.set_mocap_quat("robot_mocap", gripper_rotation)
        for _ in range(10):
            self.sim.step()
...
class MujocoFetchEnv(get_base_fetch_env(MujocoRobotEnv)):
    ...
    def _step_callback(self):
        if self.block_gripper:
            self._utils.set_joint_qpos(
                self.model, self.data, "gripper_jaw1_joint", 0.0
            )
            self._utils.set_joint_qpos(
                self.model, self.data, "gripper_jaw2_joint", 0.0
            )
    ...
    def generate_mujoco_observations(self):
        ...
        grip_velp = (
            self._utils.get_site_xvelp(self.model, self.data, "grip") * dt
        )
