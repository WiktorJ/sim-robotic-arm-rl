...
from examples.ppo.rollout_buffer import Batch, \
    RolloutBuffer, \
    EnvRolloutBuffer, \
    BatchWithProbs
...
def _calculate_advantage(values: jnp.ndarray,
                         rewards: jnp.ndarray,
                         masks: jnp.ndarray,
                         gamma: float,
                         lambda_: float) -> jnp.ndarray:
    def adv_rec2(next_adv: jnp.floating, delta_mask: jnp.ndarray) -> \
    ...
    return jax.lax.scan(
        adv_rec2, 0, jnp.flip(jnp.stack((deltas, masks), axis=-1))
    )[1]


def update_value_function(prev_values: jnp.ndarray, advantages: jnp.ndarray,
...
@functools.partial(jax.jit,
                   static_argnames=(
                           ...
                           'entropy_coef'))
def train_step_jit(batch: BatchWithProbs, seed: jax.Array,
                   policy: TrainState,
                   value_function: TrainState, gamma: float,
                   lambda_: float,
                   epsilon: float, entropy_coef: float):
    # TODO: Perhaps advantages should also be calc only once using old
    #  value_f and reused during each epoch.
    values = jnp.squeeze(jax.vmap(
        lambda obs: value_function.apply_fn(value_function.params, obs,
                                            training=True))(
        batch.observations))
    advantages = _calculate_advantage(values,
                                      batch.rewards, batch.masks, gamma,
                                      lambda_)

    policy, policy_info = PpoPolicy.update(seed, batch.observations,
                                           advantages,
                                           batch.log_probs, policy,
                                           epsilon,
                                           entropy_coef)
...
class Trainer:
    ...
    def __init__(self, config: Config):
        ...
        self.eval_env = gym.make(
            self.config.env_name, render_mode='rgb_array',
            max_episode_steps=self.config.max_episode_steps)
        self.eval_env.reset()
        ...
        self.rng, critic_seed, policy_seed = jax.random.split(
            self.rng, 3)
        value_function_def = MLP(dims=(*self.config.hidden_dims, 1),
                                 dropout_rate=self.config.dropout_rate)
        value_function_params = value_function_def.init(critic_seed,
                                                        observation)
        ...
        policy_def = PpoPolicy(hidden_dims=self.config.hidden_dims,
                               action_dim=action.shape[-1],
                               dropout_rate=self.config.dropout_rate)
        policy_params = policy_def.init(policy_seed, observation)
        self.policy = TrainState.create(
            apply_fn=policy_def.apply,
            params=policy_params, tx=optax.adam(learning_rate=self.config.lr))
    ...
    def train(self):
        ...
        (states, _), terminations = \
            self.envs.reset(), [False] * self.config.n_envs

        observation_space = self.envs.observation_space['observation'] \
            if isinstance(self.envs.observation_space,
                          gym.spaces.dict.Dict) \
            else self.envs.observation_space

        env_rollout_buffer = EnvRolloutBuffer(self.envs.action_space,
                                              observation_space,
        ...
        for i in tqdm.tqdm(range(1, self.config.max_steps + 1),
            ...
            for _ in range(self.config.rollout_length):
                ...
                states, rewards, terminations, truncations, infos = \
                    self.envs.step(actions)
                env_rollout_buffer.insert(observations, actions, rewards,
        ...
        for epoch in range(self.config.epochs):
            for batch in rollout_buffer.get(self.config.batch_size):
                self.policy, self.value_function, info = train_step_jit(
                    batch, seed, self.policy, self.value_function,
                    self.config.gamma, self.config.lambda_,
                    self.config.epsilon, self.config.entropy_coef,
                )
                infos.append(info)
        return infos
